...
**Мобильная адаптация — обязательное требование**: каждый экран и компонент делается responsive (Tailwind `sm:`/`md:`); на тач-устройствах hover-эффекты дублируются через `@media (hover: none)` (например, точки-коннекторы нод видимы всегда), сайдбар на телефоне — во всю ширину, длинное нажатие по ноде = контекстное меню.
