...
export const STR = {
  ...
  noticeImportant: 'Важно:',
  resetLinkNotice:
    'ссылка действует 1 час и срабатывает один раз. Открыть её можно на любом устройстве и в любом браузере — в том числе прямо из почтового приложения. Если не сработала — запросите новую.',
  confirmLinkNotice:
    'ссылку лучше открыть в том же браузере, где вы регистрировались — тогда вход выполнится сам. Если откроете в другом (например, из почтового приложения), почта всё равно подтвердится — просто войдите по email и паролю.',
